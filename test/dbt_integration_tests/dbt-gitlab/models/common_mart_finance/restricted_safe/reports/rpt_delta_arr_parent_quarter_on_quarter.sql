...
py_arr_with_cy_parent AS (
  ...
  LEFT JOIN dim_crm_account_daily_snapshot
    ...
    AND dim_crm_account_daily_snapshot.snapshot_date = CASE WHEN delta_arr_month < '2024-03-01'
                                                            THEN DATEADD('month', 3, snapshot_date_fpa)
                                                            ELSE DATEADD('month', 3, snapshot_date_fpa_fifth) 
                                                            END
...
final_arr AS (
  ...
  SELECT
    ...
    END                                                                                                                                                                                        AS current_quarter_arr_band,
    CASE
    ...
    SUM(ZEROIFNULL(py_arr_with_cy_parent.py_quantity))                                                                                                                                         AS prior_quarter_quantity,
    SUM(ZEROIFNULL(cy_arr_with_cy_parent.retained_quantity))                                                                                                                                   AS current_quarter_quantity
  ...
  SELECT
    ...
    CASE
      WHEN
        ...
        AND prior_quarter_quantity != current_quarter_quantity
        AND prior_quarter_quantity > 0
        THEN ZEROIFNULL(
            prior_quarter_arr / NULLIF(prior_quarter_quantity, 0) * (current_quarter_quantity - prior_quarter_quantity)
          )
      WHEN prior_quarter_quantity != current_quarter_quantity
        AND prior_quarter_quantity = 0 THEN current_quarter_arr
      ELSE 0
    END AS seat_change_arr,
    CASE
      WHEN
        prior_quarter_product_category = current_quarter_product_category
        THEN current_quarter_quantity * (
            current_quarter_arr / NULLIF(current_quarter_quantity, 0) - prior_quarter_arr / NULLIF(prior_quarter_quantity, 0)
          )
      WHEN
        prior_quarter_product_category != current_quarter_product_category
    ...
    CASE
      WHEN
        prior_quarter_product_ranking != current_quarter_product_ranking
        THEN ZEROIFNULL(
            current_quarter_quantity * (
              current_quarter_arr / NULLIF(current_quarter_quantity, 0) - prior_quarter_arr / NULLIF(prior_quarter_quantity, 0)
            )
          )
      ELSE 0
    ...
    (
      CASE
        WHEN
          ...
          THEN ZEROIFNULL(
              current_quarter_quantity * (
                current_quarter_arr / NULLIF(current_quarter_quantity, 0) - prior_quarter_quantity / NULLIF(prior_quarter_quantity, 0)
              )
            )
        ELSE 0
      END
...
SELECT
  ...
  current_quarter_product_category,
  prior_quarter_product_category,
  current_quarter_product_ranking,
  prior_quarter_product_ranking,
  prior_quarter_arr_band,
  prior_quarter_mrr,
  current_quarter_mrr,
  prior_quarter_arr,
  current_quarter_arr,
  current_quarter_arr_band,
  new_arr,
  churn_arr,
  gross_retention_arr,
  prior_quarter_quantity,
  current_quarter_quantity,
  type_of_arr_change,
