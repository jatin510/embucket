...
all_duo_monthly_seats AS (
  ...
  SELECT 
    ...
    arr.turn_on_cloud_licensing,
    CASE WHEN arr.turn_on_cloud_licensing = 'Offline' THEN 'Offline Cloud License'
         WHEN arr.turn_on_cloud_licensing = 'No' THEN 'Legacy License File'
         WHEN arr.turn_on_cloud_licensing = 'Yes' THEN 'Standard Cloud License'
         WHEN arr.turn_on_cloud_licensing = '' THEN 'Standard Cloud License'
         ELSE 'error' END
    AS license_type,
    acct.account_owner,
...
duo_and_paired_tier AS ( --tier occurring concurrently with paid duo pro subscription
  ...
  LEFT JOIN mart_arr_all AS tier -- joining to get tier occurring within same month as add on
    ...
    AND tier.is_licensed_user_base_product = TRUE
...
sm_dedicated_duo_product_info AS ( --CTE purpose is to get product info about sm and dedicated instances even if they don't trigger AI Gateway events
  ...
  LEFT JOIN map_installation_subscription_product AS s
    ...
    AND DATE_TRUNC(month, s.date_actual) = duo.reporting_month
  LEFT JOIN mart_ping_instance AS m
    ON m.dim_installation_id = s.dim_installation_id
    AND m.ping_created_date_month = DATE_TRUNC(month, s.date_actual)
...
final AS (
  ...
  LEFT JOIN wk_rpt_ai_gateway_events_flattened_with_features AS aigw_events
    ...
    AND DATE_TRUNC(month, aigw_events.behavior_date)::DATE = a.reporting_month
  GROUP BY ALL
