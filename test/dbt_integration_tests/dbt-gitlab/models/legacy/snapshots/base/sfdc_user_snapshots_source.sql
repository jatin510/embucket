WITH source AS (
    ...
    SELECT
      ...
      IFF(LOWER({{ sales_hierarchy_sales_segment_cleaning('user_segment__c') }}) = LOWER('pubsec'), CONCAT(user_geo__c, '-', UPPER({{ sales_hierarchy_sales_segment_cleaning('user_segment__c') }})), user_geo__c) AS user_geo_pubsec_segment,
