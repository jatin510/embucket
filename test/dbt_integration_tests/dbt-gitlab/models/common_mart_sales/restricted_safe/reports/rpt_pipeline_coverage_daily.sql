{{ simple_cte([
    ...
    ('actuals', 'mart_crm_opportunity_daily_snapshot'),
    ('dim_date', 'dim_date'),
...
live_actuals AS (
  ...
  */
  ...
  SELECT 
    date_actual,
    date_id,
    first_day_of_fiscal_quarter AS fiscal_quarter_date,
    last_day_of_fiscal_quarter,
    day_of_fiscal_quarter,
    fiscal_quarter_name_fy AS fiscal_quarter_name
  FROM dim_date
  WHERE 
    date_actual::DATE >= '2020-02-01'
...
base AS (
  ...
  */
  ...
  INNER JOIN snapshot_dates
    ON combined_data.snapshot_fiscal_quarter_name = snapshot_dates.fiscal_quarter_name
