...
PLEASE USE SFDC_OPPORTUNITY_SOURCE OR PREP_CRM_OPPORTUNITY
*/
...
renamed AS (
  ...
  SELECT
    ...
    number_of_sa_activity_tasks__c                                                   AS number_of_sa_activity_tasks,
    ...
    IFF(LOWER({{ sales_hierarchy_sales_segment_cleaning('user_segment_o__c') }}) = LOWER('pubsec'),CONCAT(stamped_user_geo__c, '-', UPPER({{ sales_hierarchy_sales_segment_cleaning('user_segment_o__c') }})), stamped_user_geo__c) AS user_geo_pubsec_segment_stamped,
