WITH final AS (
    ...
    SELECT
        ...
        CASE
          WHEN sfdc_contact_id IS NOT NULL
            THEN {{ dbt_utils.generate_surrogate_key(['sfdc_contact_id']) }}
            ELSE {{ dbt_utils.generate_surrogate_key(['sfdc_lead_id']) }}
          END AS dim_crm_person_id,
        email_hash,
        sfdc_lead_id,
        sfdc_contact_id,
        job_title AS title,
