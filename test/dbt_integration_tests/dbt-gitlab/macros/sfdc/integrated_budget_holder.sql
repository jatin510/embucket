{%- macro integrated_budget_holder(budget_holder,utm_budget,utm_campaign,utm_medium,campaign_rep_user_role_name) -%}
    ...
    CASE 
      ...
      WHEN LOWER({{ utm_campaign }}) LIKE '%abm%' 
        THEN 'Account Based Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%pmg%' 
        THEN 'Digital Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%fmm%' 
        THEN 'Field Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%dmp%' 
        THEN 'Digital Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%brand%' 
        THEN 'Brand Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%comm%' 
        THEN 'Brand Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%cmp%' 
        THEN 'Campaigns Team'
      WHEN LOWER({{ utm_campaign }})  LIKE '%corp%' 
        THEN 'Corporate Events'
      WHEN LOWER({{ utm_campaign }})  LIKE '%lfc%' 
        THEN 'Lifecycle Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%ptnr%' 
        THEN 'Partner Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%partner%' 
        THEN 'Partner Marketing'
      WHEN LOWER({{ utm_campaign }})  LIKE '%mdf%' 
        THEN 'Partner Marketing'
      WHEN LOWER({{ utm_medium }})  IN ('paidsocial','cpc') 
        THEN 'Digital Marketing'
      WHEN LOWER({{ utm_campaign }}) LIKE '%devopsgtm_' 
        THEN 'Digital Marketing'
